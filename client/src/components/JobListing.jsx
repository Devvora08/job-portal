  ...
  return (
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
      ...
      <div className='w-full lg:w-1/4 bg-white px-4'>
        {/* Search filter from hero component displayed here */}
        {
          ...
          <div className='flex items-center justify-center space-x-2 mt-10'>
            ...
            {Array.from({ length: Math.ceil(filteredJobs.length / 6) }).map((_, index) => (
              <a href="#job-list" key={index}>
                <button onClick={()=> setCurrentPage(index+1)} className={`w-10 flex items-center h-10 justify-center border border-gray-300 rounded ${currentPage === index+1 ? 'bg-blue-950 text-white' : 'text-gray-500'}`} >{index + 1}</button>
              </a>
            ))}
             <a href="#job-list">
