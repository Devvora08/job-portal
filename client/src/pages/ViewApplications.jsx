...
const ViewApplications = () => {
  ...
  return applicants ? applicants.length === 0 ? (
    ...
    <div className='container mx-auto p-4'>
      <div>
        <table className='w-full max-w-4xl bg-white border rounded border-gray-200 max-sm:text-sm'>
          <thead>
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr className='text-gray-700' key={index}>
                ...
                <td className='py-2 px-4 border-b'>
                  <a className='bg-blue-50 text-blue-800 px-3 py-1 rounded inline-flex gap-2 items-center' href={applicant.userId.resume} target='_blank'>
                    Resume <img src={assets.resume_download_icon} alt="" />
                  </a>
                </td>
                <td className='py-2 px-4 borber-b relative'>
